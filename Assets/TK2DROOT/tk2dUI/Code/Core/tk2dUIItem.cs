...
public class tk2dUIItem : MonoBehaviour
{
    ...
    public bool Press(tk2dUITouch touch, tk2dUIItem sentFromChild) //pressed down ontop of button
    {
        ...
        if (!isPressed)
        {
            ...
            if (registerPressFromChildren || sentFromChild == null)
            {
                ...
                {
                    isPressed = true;
					if (isChildOfAnotherUIItem == false && sendMessageTarget != null && SendMessageOnClickMethodName != "") {
						// if we're not a child of another ui item, play the button sound on down
						AudioPlayer.instance.PlayFirstSound(1f, AudioPlayer.SoundType.Button_Click);
					}
    ...
    public void Release()
    {
        if (isPressed)
        {
			...
			if (isChildOfAnotherUIItem == true && sendMessageTarget != null && SendMessageOnClickMethodName != "") {
				// if we are a child of another ui item, play the button sound on click
				AudioPlayer.instance.PlayFirstSound(1f, AudioPlayer.SoundType.Button_Click);
			}

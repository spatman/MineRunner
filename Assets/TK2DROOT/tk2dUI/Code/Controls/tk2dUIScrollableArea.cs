...
public class tk2dUIScrollableArea : MonoBehaviour
{
    ...
    void OnDisable()
    {
        ...
        if (true || isBackgroundButtonDown)
        {
            if (tk2dUIManager.Instance != null)
            {
    ...
    Vector3 ContentContainerOffset {
        ...
        set {
			if (contentContainer == null || contentContainer.transform == null) {
    ...
    private void BackgroundOverUpdate()
    {
        ...
        if (isSwipeScrollingInProgress)
        {
            ...
            if (scrollBar != null)
            {
                ...
                if (scrollAxes == Axes.XAxis)
                {
                    Value = (ContentContainerOffset.x / (contentLength - visibleAreaLength));
                }
                else if (scrollAxes == Axes.YAxis)
                {
                    Value = (ContentContainerOffset.y / (contentLength - visibleAreaLength));

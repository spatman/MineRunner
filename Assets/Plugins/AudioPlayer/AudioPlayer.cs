...
public class AudioPlayer : MonoBehaviour {
	...
	[Range(0f, 1f)] public float _masterVolumeMusic = 1f;
	[Range(0f, 1f)] public float _masterVolumeSound = 1f;
	...
	public enum SoundType {
		...
		//special coin sounds
		Special_Coin_Appear = 700,
		Special_Coin_Collect
	}
...
//		}
	...
	public void PlayFirstSound(float volume, SoundType soundType, float delay = 0f, System.Action<AudioSource> sourceCb = null, float pitch = 1f, float jumpToTimeInClip = 0f) {
		// get sound info
		//Debug.Log(soundType);
		AudioPlayerClipCollectionData soundData;
		try {
			soundData = _soundMap[soundType];
		} catch (KeyNotFoundException) {
			Debug.Log("sound type not found: " + soundType);
			if (sourceCb != null) {
				sourceCb(null);
			}
			return;
